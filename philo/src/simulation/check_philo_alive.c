...
/*   Updated: 2025/04/07 10:53:26 by poverbec         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	check_philo_alive(t_philo *philo, t_program *program)
{
	...
	if (program->time_to_die < (get_current_time(program) \
...
/* 
printf("philo id %d time to die %ld | difference fresh %ld  | current time %ld \
- last_time eaten %ld \n",
		// philo->philo_id, program->time_to_die,(get_current_time(program) \
		- philo->time_last_eaten),
